...
import type { Campaign } from '@/entities/Campaign'; // Interface Campaign
// Importa a função real de listagem de campanhas passando para o alias abaixo:
...
export default function BudgetPage() {
  ...
  // Estilos (podem ser ajustados conforme tema)
  const neonColor = '#1E90FF';
  const neonRedColor = '#FF4444';
  const cardStyle = "bg-[#141414]/80 backdrop-blur-sm shadow-[5px_5px_10px_rgba(0,0,0,0.4),-5px_-5px_10px_rgba(255,255,255,0.05)] rounded-lg border-none p-4";
  const insetCardStyle = "bg-[#141414]/50 shadow-[inset_1px_1px_2px_rgba(0,0,0,0.3),inset_-1px_-1px_2px_rgba(255,255,255,0.03)] rounded-md border-none p-3";
  ...
  const neumorphicSliderStyle = "[&>span:first-child]:bg-[#141414] [&>span:first-child]:shadow-[inset_2px_2px_4px_rgba(0,0,0,0.3),inset_-2px_-2px_4px_rgba(255,255,255,0.05)] [&>span:first-child]:h-2 [&>span>span]:shadow-[3px_3px_6px_rgba(0,0,0,0.3),-3px_-3px_6px_rgba(255,255,255,0.05)] [&>span>span]:h-4 [&>span>span]:w-4 [&>span>span]:border-none";
  const primaryIconStyle = { filter: `drop-shadow(0 0 3px ${neonColor})` };
  const labelStyle = "text-xs text-gray-300 mb-0.5 block";
  const titleStyle = "text-lg font-semibold text-white";

  // Função para recalcular alocação baseada nos inputs
  const calculateAllocationCallback = useCallback(() => {
    ...
    if (totalAllocatedPercentage > 100) {
      setError(`A soma das alocações (${totalAllocatedPercentage}%) excede 100%. Ajuste os valores.`);
    }
    
    setCalculatedValues({ 
      trafficCost: traffic, 
      creativeCost: creative, 
      operationalCostValue: operational, 
    ...
    if (unallocatedValue > 0.01) {
      dataForChart.push({ name: 'Não Alocado', value: unallocatedValue, percentage: unallocatedPercentage, color: COLORS[4] });
    }
    setAllocationData(dataForChart.filter(item => item.value > 0.01));
  }, [totalBudget, trafficInvestment, creativeInvestment, operationalCost, profitMargin]);

  useEffect(() => {
    calculateAllocationCallback();
  }, [calculateAllocationCallback]);

  // Função mock para listagem de campanhas (fallback)
  ...
  useEffect(() => {
    const fetchCampaignData = async () => {
      try {
        // Tenta usar a função real importada; se não existir, usa o mock
    ...
    // Se desejar usar a função de fetch, descomente a chamada:
    // fetchCampaignData();
  ...
  return (
    <Layout>
      <Head>
        <title>Planejamento Orçamentário - USBMKT</title>
      </Head>
          ...
          <div className="lg:col-span-1 space-y-4">
            <Card className={cn(cardStyle)}>
              ...
              <CardContent className="p-0">
                <Label htmlFor="totalBudget" className={labelStyle} style={{ textShadow: `0 0 4px ${neonColor}` }}>
                  Valor Total (R$)
                </Label>
                <Input
                  id="totalBudget"
                  ...
                  min="0"
                  step="0.01"
                  className={cn(neumorphicInputStyle, "mt-1")}
                />
              </CardContent>
            </Card>
              ...
              <CardContent className="p-0 space-y-4">
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center">
                    <Label htmlFor="trafficInvestment" className={cn(labelStyle, "flex items-center gap-1")} style={{ textShadow: `0 0 4px ${neonColor}` }}>
                      <TrendingUp className="h-4 w-4" style={{ filter: `drop-shadow(0 0 3px ${COLORS[0]})` }} /> Tráfego
                    </Label>
                    <span className="text-xs font-medium text-white w-10 text-right">{trafficInvestment}%</span>
                  </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center">
                    <Label htmlFor="creativeInvestment" className={cn(labelStyle, "flex items-center gap-1")} style={{ textShadow: `0 0 4px ${neonColor}` }}>
                      <Target className="h-4 w-4" style={{ filter: `drop-shadow(0 0 3px ${COLORS[1]})` }} /> Criativos
                    </Label>
                    <span className="text-xs font-medium text-white w-10 text-right">{creativeInvestment}%</span>
                  </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center">
                    <Label htmlFor="operationalCost" className={cn(labelStyle, "flex items-center gap-1")} style={{ textShadow: `0 0 4px ${neonColor}` }}>
                      <ClipboardList className="h-4 w-4" style={{ filter: `drop-shadow(0 0 3px ${COLORS[2]})` }} /> Operacional
                    </Label>
                    <span className="text-xs font-medium text-white w-10 text-right">{operationalCost}%</span>
                  </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center">
                    <Label htmlFor="profitMargin" className={cn(labelStyle, "flex items-center gap-1")} style={{ textShadow: `0 0 4px ${neonColor}` }}>
                      <DollarSign className="h-4 w-4" style={{ filter: `drop-shadow(0 0 3px ${COLORS[3]})` }} /> Lucro
                    </Label>
                    <span className="text-xs font-medium text-white w-10 text-right">{profitMargin}%</span>
                  </div>
              ...
              <CardContent className="p-0 h-[300px]">
                {totalBudget > 0 && allocationData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        ...
                        cy="50%"
                        labelLine={false}
                        label={({ name, percentage, x, y, midAngle, innerRadius, outerRadius }) => {
                          const RADIAN = Math.PI / 180;
                          const radius = innerRadius + (outerRadius - innerRadius) * 1.3;
                          const lx = x + radius * Math.cos(-midAngle * RADIAN);
                          const ly = y + radius * Math.sin(-midAngle * RADIAN);
                          return (
                            <text x={lx} y={ly} fill="hsl(var(--muted-foreground))" textAnchor={lx > x ? 'start' : 'end'} dominantBaseline="central" fontSize={10}>
                  ...
                  <div className="flex items-center justify-center h-full text-gray-500 text-sm italic">
                    {totalBudget <= 0 ? "Insira um orçamento total." : "Ajuste as alocações."}
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <Card className={cn(cardStyle)}>
              ...
              <CardContent className="p-0 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className={cn(insetCardStyle)}>
                  ...
                  <span className="block text-right font-semibold text-white text-sm">{formatCurrency(calculatedValues.trafficCost)}</span>
                </div>
                <div className={cn(insetCardStyle)}>
                  <span className="text-xs font-medium text-gray-300 flex items-center gap-1.5">
                    <Target className="h-4 w-4" style={{ filter: `drop-shadow(0 0 3px ${COLORS[1]})` }} /> Criativos
                  </span>
                  <span className="block text-right font-semibold text-white text-sm">{formatCurrency(calculatedValues.creativeCost)}</span>
                </div>
                <div className={cn(insetCardStyle)}>
                  <span className="text-xs font-medium text-gray-300 flex items-center gap-1.5">
                    <ClipboardList className="h-4 w-4" style={{ filter: `drop-shadow(0 0 3px ${COLORS[2]})` }} /> Operacional
                  </span>
                  <span className="block text-right font-semibold text-white text-sm">{formatCurrency(calculatedValues.operationalCostValue)}</span>
                </div>
                <div className={cn(insetCardStyle)}>
                  <span className="text-xs font-medium text-gray-300 flex items-center gap-1.5">
                    <DollarSign className="h-4 w-4" style={{ filter: `drop-shadow(0 0 3px ${COLORS[3]})` }} /> Lucro
                  </span>
                  <span className="block text-right font-semibold text-white text-sm">{formatCurrency(calculatedValues.expectedProfit)}</span>
                </div>
                {calculatedValues.unallocated > 0.01 && (
                  <div className={cn(insetCardStyle, "sm:col-span-2 bg-[#141414]/30 border border-dashed border-[#9370DB]/50")}>
                    <span className="text-xs font-medium text-gray-400">Não Alocado</span>
                    <span className="block text-right font-semibold text-gray-400 text-sm">{formatCurrency(calculatedValues.unallocated)}</span>
                  </div>
                )}
              </CardContent>
            </Card>
